<script lang="ts">
  import CodeSnippet from "$lib/app/snippet/code-snippet.svelte";
  ...
  interface TabCodeSnippetsProps {
    value: string;
    items: TabCodeSnippetProps[];
  }

  let { value, items }: TabCodeSnippetsProps = $props();
</script>

<Tabs {value} class="w-full max-w-screen">
