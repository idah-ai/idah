<script lang="ts">
  ...
  import { SearchIcon, XCircleIcon, FileTextIcon, PlayCircleIcon, DownloadIcon, PuzzleIcon, CodeIcon, SparklesIcon } from "@lucide/svelte";
  ...
  let mockResults = [
    {
      url: "/plugin/create-plugin/",
      title: "Create a Plugin",
      excerpt: "Learn how to create custom <mark>plugins</mark> for IDAH. This guide covers the plugin structure, development workflow, and best practices."
    },
    {
      url: "/getting-started/about/",
      title: "About IDAH",
      excerpt: "IDAH is an open-source platform for collaborative data annotation, designed to streamline the creation of high-quality training datasets."
    },
    {
      url: "/install/",
      title: "Installation",
      excerpt: "Follow these steps to install and configure IDAH on your system. Supports Docker, local development, and production deployments."
    }
  ];
...
<div class="relative flex w-full items-center justify-center lg:px-0">
  ...
  >
    <InputGroup class="w-full relative z-50 bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-300 dark:border-gray-600">
    ...
    {#if isFocused && !$searchQuery}
      <div class="absolute top-full left-0 right-0 -mx-4 md:mx-0 md:left-auto md:right-auto md:w-[400px] lg:w-[500px] md:mt-2 bg-white dark:bg-gray-800 border-t md:border border-gray-200 dark:border-gray-700 md:rounded-xl shadow-2xl z-50 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-gray-800 dark:to-gray-750">
              ...
              >
                <div class="flex items-start gap-3">
                  <div class="p-2 rounded-lg {topic.bgColor} group-hover:scale-110 transition-transform">
                    <svelte:component this={topic.icon} class="w-4 h-4 flex-shrink-0" style="color: {topic.iconColor};" />
                  </div>
                  <div class="flex-1">
                    <div class="text-sm font-semibold text-gray-900 dark:text-white mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                      {topic.title}
                    </div>
    ...
    {#if isFocused && $searchQuery}
      <div class="absolute top-full left-0 right-0 -mx-4 md:mx-0 md:left-auto md:right-auto md:w-[400px] lg:w-[500px] md:mt-2 bg-white dark:bg-gray-800 border-t md:border border-gray-200 dark:border-gray-700 md:rounded-lg shadow-xl max-h-[400px] overflow-y-auto z-50">
        {#if isSearching}
