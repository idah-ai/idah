...
export interface ICommandDriverV2 {
  /**
   * Register a command.
   * @param opts.name      Unique command name.
   * @param opts.modes     Array of mode names where this command is active.
   * @param opts.shortcut  Optional keyboard shortcut — a canonical key-combination string
   *                  (e.g. `"Control+Z"`, `"Delete"`, `"Control+Shift+Z"`).
   * @param opts.shortDescription Short description (displayed in menus).
   * @param opts.longDescription  Long description.
   * @param opts.callback  Factory that returns an ICommandAction when called.
   * @param opts.group     Optional group/category label for the palette.
   * @param opts.activeWhen Optional predicate: only active when returns true.
...
// Plugin interface
export interface IPluginDriver {
