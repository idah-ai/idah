<script lang="ts">
  ...
  onMount(() => {
    ...
    // Two reasons: (1) it loads shortcut overrides into the live map that
    // CommandManagerV2 already references, so remaps apply immediately; (2) the
    // plugin reads its persisted settings (e.g. category label visibility)
    // synchronously in init() via hydrateSettings, and reactivity does not cross
    // the plugin/core bundle boundary — so the value must already be present.
    // Guarded so a settings failure never blocks plugin startup.
    driver.accountSettings
      .load($authStatus.authContext?.id ?? "")
      .catch((err) => {
        console.error("Failed to load account settings; continuing with defaults", err);
      })
      .then(() => {
        p.then((_plugin) => {
          plugin = _plugin;
