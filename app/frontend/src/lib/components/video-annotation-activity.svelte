<script lang="ts">
  ...
  function selectAnnotation(annotation?: VideoAnnotation) {
    ...
    if (selectedAnnotation && annotation) {
      updateAnnotationValue(selectedAnnotation, annotation.value);
    }
  }
</script>
    ...
    <ResizablePane class="flex h-full" defaultSize={60} minSize={10}>
      <SidebarProvider>
        <AnnotationSidebar
          ...
          onSelectSubCategory={({ selectedMode, annotationValue: value }) => {
            annotationValue = value;
            mode = selectedMode;
            if (selectedAnnotation) {
              updateAnnotationValue(selectedAnnotation, annotationValue);
            }
          }}
