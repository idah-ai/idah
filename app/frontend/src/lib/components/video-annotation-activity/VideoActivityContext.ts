...
import { testVideoAnnotationContext } from "@/components/video-annotation-activity/VideoAnnotationContext";

const videoActivityContext: ActivityContext = {
	mode: "edit",
	medias: {
		main: {
			url: "src/assets/disney.mp4 ",
	...
	tools: {
		"bounding-box": {
			...
			allowedFields: {
				categories: {
					vehicles: [{ name: "car" }, { name: "truck" }, { name: "motorcycle" }, { name: "bicyle" }],
					pedestrians: [{ name: "adult" }, { name: "child" }],
					traffic_signs: [{ name: "stop" }, { name: "yield" }, { name: "speed_limit" }, { name: "crosswalk" }],
				},
				label: true,
				text: true,
				confidence: true,
			...
			allowedFields: {
				categories: {
					object: [
						{
							name: "pedestrians",
							nestedCategories: [{ name: "adult" }, { name: "child" }],
						},
					],
				},
			},
