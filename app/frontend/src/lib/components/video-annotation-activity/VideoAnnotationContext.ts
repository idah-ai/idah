...
export const videoModes = ["view", "bounding-box", "bounding-polygon"] as const;
...
export let testVideoAnnotationContext: VideoAnnotationContext = {
  ...
  addAnnotation(annotation: VideoAnnotation): Promise<void> {
    return new Promise((resolve, reject) => {
      if (annotations.length + 1 == annotations.push(annotation)) resolve();
      else reject();
    });
  },
  removeAnnotation(id: string): Promise<void> {
            ...
            CommandManager.add({
              ...
              apply() {
                v.shape = {
                  ...
                  frames: [...v.shape.frames, selection],
                };
              },
              undo() {
