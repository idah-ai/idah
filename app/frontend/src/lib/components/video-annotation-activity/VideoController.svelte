<script lang="ts">
	...
	let { video = $bindable(), currentFrame, totalFrames }: Props = $props();
...
<div id="video-controller" class="flex w-full items-center justify-between gap-4">
	...
	<!-- CONTAINER::RIGHT -->
	<div class="flex items-center gap-2">
		<!-- VIDEO::SCALE ADJUSTER (ZOOM IN / ZOOM OUT) -->
		<Popover>
			<PopoverTrigger>
				<Button variant="outline" size="icon">
			...
			<PopoverContent side="top" align="center" class="max-w-10 px-2">
				<Slider type="single" orientation="vertical" min={20} max={150} step={1} bind:value={scale} />
