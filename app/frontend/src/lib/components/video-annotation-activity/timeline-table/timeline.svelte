...
<div class='' style:height=3em >
        ...
        <TimelineCell
            ...
            inSpan={
                Math.floor(
                    (annotation.shape.start - range[0]) / timeline_zoom
                ) <= i &&
                Math.floor(
                    (annotation.shape.end - range[0]) / timeline_zoom
                ) >= i
            }
            {onSeekFrame}
            keyframes={
                annotation.shape.frames.filter(s => Math.floor((s.frame - range[0]) / timeline_zoom) == i).map(s => s.frame)
            }
            onDeleteFrame={(frame) => onDeleteAnnotation(annotation, frame)}
            hovered={hovered_column == range[0] + i * timeline_zoom}
            onmouseover={() => { hoveredColumnChange(range[0] + i * timeline_zoom) }}
            onmouseenter={() => { hoveredColumnChange(range[0] + i * timeline_zoom) }}
            onmouseleave={() => { hoveredColumnChange(undefined) }}
            {...restProps}
        />
        {/each}
  </div>