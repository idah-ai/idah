...
This directory provides a standalone mock environment for developing the **idah-video** plugin without needing the full IDAH backend stack.

## Quick Start

1. **Place your video file** in `static/sample.m3u8` (or any path the mock context points to).

   You can also edit `src/mock/context.ts` and change the `mediaUrl` getter to point to any video source (e.g., a remote HLS stream).

2. **Start the dev server**:

   ```bash
   cd plugins/idah-video/frontend
   pnpm dev
   ```
...
| Media | Video URL from `mediaUrl` getter; metadata returns 250 frames at 25 fps |
...
```ts
get mediaUrl() {
  return "http://your-server/path/to/video.m3u8";
  // or use a local file in static/:
  // return "/medias/my-video.m3u8";
}
```

Also update the `mediaInfo()` method to match your video's actual duration and fps:
...
Edit the `config` getter. The config is keyed by shape type (`"idah-video:bounding-box"`, `"idah-video:polygon"`):
