<script lang="ts">
  ...
  onMount(() => {
    ...
    async function setPenPlusCursor() {
      const cursorPenPlusSvgText = await context.icons.get("pen-tool-add-2");
      const encoded = encodeURIComponent(cursorPenPlusSvgText);
      const dataUrl = `data:image/svg+xml,${encoded}`;

      const style = document.createElement("style");
      style.textContent = `
        .cursor-pen-plus {
          cursor: url(${dataUrl}) 0 0, auto;
        }
      `;
    ...
    async function setPenRemoveCursor() {
      const cursorPenRemoveSvgText = await context.icons.get("pen-tool-remove-2");
      const encoded = encodeURIComponent(cursorPenRemoveSvgText);
      const dataUrl = `data:image/svg+xml,${encoded}`;
