<script lang="ts">
  ...
  import { VIDEO_FACIAL_LANDMARK } from "$lib/types";
  ...
  let { cursor, mediaWidth, mediaHeight, frame, onSelection, color = "#22c55e" }: Props = $props();
  ...
  export function handleMouseUp(c: Point): boolean {
    ...
    if (Math.abs(c[0] - start[0]) * mediaWidth < 10 ||
        Math.abs(c[1] - start[1]) * mediaHeight < 10) return false;
    ...
    onSelection(VIDEO_FACIAL_LANDMARK, frame, fitted as Point[], 0, undefined);
    return true;
  }
