<script lang="ts">
  ...
  import ColorPicker from "$lib/components/app/color-picker/color-picker.svelte";
  import FormFieldErrors from "$lib/components/app/forms/form-field-errors.svelte";
  import FormFieldInfo from "$lib/components/app/forms/form-field-info.svelte";
  import FormFieldLabel from "$lib/components/app/forms/form-field-label.svelte";
  import FormField from "$lib/components/app/forms/form-field.svelte";
  ...
  import type { FormFieldBaseProps } from "$lib/components/app/forms/form-field.types";
...
<FormField id={name} class={cn("", className)}>
  ...
  <Popover>
    <PopoverTrigger>
      {#snippet child({ props })}
        <div class="flex w-full items-center gap-2">
          ...
          {#if value}
            <Text size="sm" weight="semibold" class="text-muted-foreground"
              >{detectColorFormat(value)}</Text
            >
    ...
    <PopoverContent class="w-auto p-0" side="right">
      <ColorPicker bind:value onValueChange={handleValueChange} {slotSuggestion}
      ></ColorPicker>
    </PopoverContent>
  </Popover>
