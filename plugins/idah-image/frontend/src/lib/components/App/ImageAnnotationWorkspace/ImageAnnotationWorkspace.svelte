<script lang="ts">
  ...
  import { DEFAULT_MODE, IMAGE_BOUNDING_BOX as IDAH_IMAGE_BOUNDING_BOX, IMAGE_CIRCLE as IDAH_IMAGE_CIRCLE, IMAGE_ELLIPSE as IDAH_IMAGE_ELLIPSE, IMAGE_LINE as IDAH_IMAGE_LINE, IMAGE_POLYGON as IDAH_IMAGE_POLYGON, IMAGE_BOUNDING_BOX, IMAGE_CIRCLE, IMAGE_ELLIPSE, IMAGE_LINE, IMAGE_POLYGON, IMAGE_MASK, IMAGE_FACIAL_LANDMARK, NOTE_MODE, REVIEW_MODE } from "$lib/types";
  ...
  $effect(() => {
    ...
    if (viewportMode !== IMAGE_BOUNDING_BOX && viewportMode !== IMAGE_CIRCLE && viewportMode !== IMAGE_ELLIPSE && viewportMode !== IMAGE_LINE && viewportMode !== IMAGE_POLYGON && viewportMode !== IMAGE_MASK && viewportMode !== IMAGE_FACIAL_LANDMARK) {
      pendingValue = {};
    }
  ...
  onMount(async () => {
    ...
    const toolListConfig = [
      ...
      {
        name: "tools.facial_landmark",
        label: "Facial Landmark",
        type: IMAGE_FACIAL_LANDMARK,
        iconName: "facial-landmark",
        disabled: !editable,
        command: "tools.facial_landmark",
      },
      {
        name: "tools.note",
        label: "Add Note",
    ...
    const toolConfig = toolListConfig.filter((tool) => {
      if ([IDAH_IMAGE_BOUNDING_BOX, IDAH_IMAGE_CIRCLE, IDAH_IMAGE_ELLIPSE, IDAH_IMAGE_LINE, IDAH_IMAGE_POLYGON, IMAGE_FACIAL_LANDMARK].includes(tool.type)) {
