...
import { IMAGE_BOUNDING_BOX, IMAGE_POLYGON } from "$lib/types";
...
export function nearestKeyframe(
  ...
  const sorted = [...frames].sort((a, b) => Math.abs(a.frame - frame) - Math.abs(b.frame - frame));
...
function lerpPoints(a: Point[], b: Point[], t: number): Point[] {
  return a.map((p, i) => [p[0] + (b[i][0] - p[0]) * t, p[1] + (b[i][1] - p[1]) * t] as Point);
}
...
function getSurroundingKeyFrames(
  ...
  current: number,
): [IImageFrameSelection, IImageFrameSelection] | null {
  let before: IImageFrameSelection | null = null;
  let after: IImageFrameSelection | null = null;
