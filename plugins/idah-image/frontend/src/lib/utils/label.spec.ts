...
describe("labelCenterPx", () => {
  describe("bounding box", () => {
    it("returns the centre of the box in media pixels", () => {
      const ann = { shape_type: IMAGE_BOUNDING_BOX, shape_args: { points: box(0.2, 0.4, 0.6, 0.8) } };
      expect(labelCenterPx(ann, W, H)).toEqual([400, 300]);
    });

    it("is unaffected by corner ordering", () => {
      const ann = { shape_type: IMAGE_BOUNDING_BOX, shape_args: { points: [[0.6, 0.8], [0.2, 0.8], [0.2, 0.4], [0.6, 0.4]] } };
      expect(labelCenterPx(ann, W, H)).toEqual([400, 300]);
    });
    ...
    it("keeps the same centre at any angle", () => {
      const unrotated = labelCenterPx({ shape_type: IMAGE_BOUNDING_BOX, shape_args: { points: squarePts } }, W, H)!;
      expect(unrotated[0]).toBeCloseTo(400, 6);
      expect(unrotated[1]).toBeCloseTo(250, 6);
      for (const angle of [0, Math.PI / 4, Math.PI / 2, Math.PI]) {
        const rotated = labelCenterPx({ shape_type: IMAGE_BOUNDING_BOX, shape_args: { points: squarePts, angle } }, W, H)!;
    ...
    it("returns null when points are missing or incomplete", () => {
      expect(labelCenterPx({ shape_type: IMAGE_BOUNDING_BOX }, W, H)).toBeNull();
      expect(labelCenterPx({ shape_type: IMAGE_BOUNDING_BOX, shape_args: { points: [[0.1, 0.1]] } }, W, H)).toBeNull();
    });
  });
  ...
  describe("ellipse", () => {
    it("returns the centroid (cx, cy) in media pixels", () => {
      const ann = { shape_type: IMAGE_ELLIPSE, shape_args: { points: [[0.3, 0.4], [0.2, 0.1]] } };
    ...
    it("ignores the radii — the centre is the first point", () => {
      const ann = { shape_type: IMAGE_ELLIPSE, shape_args: { points: [[0.5, 0.5], [0.1, 0.1]] } };
      const center = labelCenterPx(ann, W, H)!;
    ...
    it("is unchanged by rotation", () => {
      ...
      const unrotated = labelCenterPx({ shape_type: IMAGE_ELLIPSE, shape_args: { points: pts } }, W, H)!;
      const rotated = labelCenterPx({ shape_type: IMAGE_ELLIPSE, shape_args: { points: pts, angle: Math.PI / 4 } }, W, H)!;
    ...
    it("returns null when the radii point is missing", () => {
      expect(labelCenterPx({ shape_type: IMAGE_ELLIPSE, shape_args: { points: [[0.5, 0.5]] } }, W, H)).toBeNull();
      expect(labelCenterPx({ shape_type: IMAGE_ELLIPSE }, W, H)).toBeNull();
    });
  });

  describe("circle", () => {
    it("returns the centre point, ignoring the radius", () => {
      const ann = { shape_type: IMAGE_CIRCLE, shape_args: { points: [[0.5, 0.5]], radius: 0.1 } };
      expect(labelCenterPx(ann, W, H)).toEqual([500, 250]);
    });

    it("returns null with no centre point", () => {
      expect(labelCenterPx({ shape_type: IMAGE_CIRCLE, shape_args: { points: [] } }, W, H)).toBeNull();
    });
  });
  ...
  describe("polygon and line", () => {
    it("returns the polygon's visual centre", () => {
      ...
      const ann = { shape_type: IMAGE_POLYGON, shape_args: { points: [[0.3, 0.3], [0.5, 0.3], [0.5, 0.7], [0.3, 0.7]] } };
      const center = labelCenterPx(ann, W, H)!;
      expect(Math.hypot(center[0] - 400, center[1] - 250)).toBeLessThan(2);
    });

    it("returns the midpoint of the two line endpoints", () => {
      const ann = { shape_type: IMAGE_LINE, shape_args: { points: [[0.8, 0.2], [0.3, 0.9]] } };
      const center = labelCenterPx(ann, W, H)!;
    ...
    it("returns null for a degenerate line", () => {
      expect(labelCenterPx({ shape_type: IMAGE_LINE, shape_args: { points: [[0.1, 0.1]] } }, W, H)).toBeNull();
    });
  });

  describe("excluded and unknown shapes", () => {
    it("returns null for masks", () => {
      expect(labelCenterPx({ shape_type: IMAGE_MASK, shape_args: { points: [[0.1, 0.1], [0.5, 0.5]] } }, W, H)).toBeNull();
    });

    it("returns null for an unrecognised shape type", () => {
      expect(labelCenterPx({ shape_type: "idah-image:something-new", shape_args: { points: [[0.1, 0.1]] } }, W, H)).toBeNull();
    });
...
describe("annotationLabel", () => {
  ...
  it("flags a category that is set but missing from the config", () => {
    const result = annotationLabel({ category: "vehicle/commercial/bus" }, lookup);
